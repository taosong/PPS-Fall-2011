...
public class AshMapper extends Mapper {
	...
	/*
	 * This function returns only unique mapping. i.e. Each element - maps to a
	 * unique element.
	 */
	...
	public void updateGuesserAction(GuesserAction gsaGA) {
		 ...
		 */
		System.out
				.println("The elements that the guesser asked this time are :"
						+ gsaGA.getContent().toString());
		allQueries.add(gsaGA);

	}

	/*
	 * This function analyses the Guesser Action, and then returns an integer
	 * value, that helps to generate the next mapping.
	 */
	public int analyseGuesserAction() {
		...
		if (!allQueriesFromPreviousRound.isEmpty() && !firstRound) {
			...
			/*
			 * for (Iterator<GuesserAction> it = allQueries.iterator();
			 * it.hasNext();){ //GuesserAction tempGuesserAction = it.next();
			 * //ArrayList<Integer> tempContent =
			 * tempGuesserAction.getContent();
			 * 
			 * 
			 * }
			 */
			metric = GuessAnalyser.numOfTimesElementCountAppearsInAllQueries(
					allQueriesFromPreviousRound, 2);
	...
	public void notFirstRound() {
		/*
		 * Change the flag value of the first round to false.
		 */
		firstRound = false;
	}
